	...
	if !c.Force && globals.Cache != nil {
		if cached, ok := globals.Cache.Get(cacheKey); ok {
			if info, valid := cached.(UpdateInfo); valid {
				return c.displayUpdateInfo(info, c.Format)
			}
		}
	}
...
func (c *UpdateCheckCmd) displayUpdateInfo(info UpdateInfo, format string) error {
	if format == "json" {
...
func AutoUpdateCheck(cacheInstance *cache.Cache) {
	...
	if cacheInstance != nil {
		if _, ok := cacheInstance.Get(cacheKey); ok {
			return
		}
	}
	...
	go func() {
		...
		// Cache the result
		if cacheInstance != nil {
			cacheInstance.Set(cacheKey, info, cacheTTL)
		}
...
// Globals holds global configuration (needed for update check command)
type Globals struct {
	Cache *cache.Cache
}
